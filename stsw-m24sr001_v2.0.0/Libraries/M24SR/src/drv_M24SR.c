...
/**
  * @brief  This function sends the I2C Token Release
	* @retval M24SR_ACTION_COMPLETED : the function is succesful. 
	* @retval M24SR_ERROR_I2CTIMEOUT : The I2C timeout occured. 
  */
...
/**
  * @brief  This function sends the UpdateFileType command
	...
	* @retval M24SR_ERROR_I2CTIMEOUT : The I2C timeout occured.
  */
uint16_t M24SR_UpdateFileType ( u8 FileType )
{
	uint8_t 	*pBuffer = uM24SRbuffer ;
	uint16_t	status ; 
	uint16_t	NbByte ;
	...
	Command.Header.INS = C_APDU_UPDATE_FILE_TYPE;
	/* copy the offset */
	Command.Header.P1 = 0x00;
	Command.Header.P2 = 0x00;
	/* copy the number of byte of the data field */
	...
	/* copy the File Id */
	memcpy(Command.Body.pData , &FileType, 1 );
