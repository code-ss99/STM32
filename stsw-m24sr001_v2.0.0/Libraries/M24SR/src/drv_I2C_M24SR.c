...
uint16_t	NbAtempt = M24SR_I2C_POLLING,
  ...
  /*---------------------------- Transmission Phase ---------------------------*/
	 ...
	 /*!< Send M24SR64-Y slave address for write */
		...
		uSR1register = I2C_ReadRegister(M24SR_I2C, I2C_Register_SR1);
		/* Note: ADDR is not set after a NACK reception */
		while ( ((uSR1register != 0x0400) && ((uSR1register&0x0002) !=0x0002))&& --M24SR_Timeout)/*BUSY & MSL & ADDR */
		{
			/*reads the SR1 register */ 
			uSR1register = I2C_ReadRegister(M24SR_I2C, I2C_Register_SR1);
			uSR1register &= 0x0482;
		}	
		...
		if (uSR1register == 0x0002)
		{
			/* Disable the START condition generation */
			I2C_Cmd(M24SR_I2C, DISABLE);
			do
			{
				uSR1register = I2C_ReadRegister(M24SR_I2C, I2C_Register_SR1);
			}
			while( (uSR1register&0x0002) ==0x0002);
	...
	} while ( (uSR1register != 0x0000) && (--NbAtempt > 0) );
...
void M24SR_I2CInit ( void )
{
  ...
  /* GPIO Periph clock enable */
	...
	RCC_AHB1PeriphClockCmd(M24SR_GPIO_BLOCK_CLK, ENABLE);
#else
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_AFIO | M24SR_GPIO_BLOCK_CLK, ENABLE);
#endif
  ...
  GPIO_PinAFConfig(M24SR_SCL_PIN_PORT, M24SR_SCL_PIN_SOURCE, GPIO_AF_I2C2);
  GPIO_PinAFConfig(M24SR_SDA_PIN_PORT, M24SR_SDA_PIN_SOURCE, GPIO_AF_I2C2);
#endif
...
#endif
  GPIO_Init(M24SR_GPO_PIN_PORT, &GPIO_InitStructure);
...
void M24SR_RFConfig_Hard( uc8 OnOffChoice)
{
	...
	{	
	#if defined STM32F40_41xxx
		M24SR_RFDIS_PIN_PORT->BSRRH = M24SR_RFDIS_PIN;
	#else
	...
	{	
	#if defined STM32F40_41xxx
		M24SR_RFDIS_PIN_PORT->BSRRL = M24SR_RFDIS_PIN;		
	#else
